...
const TagPreview = forwardRef<HTMLDivElement, { values: DesignFormValues; user: any; side: 'front' | 'back' }>(({ values, user, side }, ref) => {
    ...
    // Lanyard Card
    if (layout === 'lanyard') {
         const holeBg = values.textColor === 'light' ? 'bg-black/20' : 'bg-white/20';
         const holeBorder = values.textColor === 'light' ? 'border-white/40' : 'border-black/40';
         return (
            <div ref={ref} className={cn("w-full h-full rounded-2xl p-6 transition-colors relative flex flex-col items-center justify-center text-center", cardBgClass)} style={cardStyle}>
                {values.backgroundImageUrl && <div className="absolute inset-0 bg-black/50 rounded-2xl" />}
                <div className={cn("absolute top-5 left-1/2 -translate-x-1/2 h-5 w-5 rounded-full border-2", holeBg, holeBorder)} />
                                            ...
                                            render={({ field }) => (
                                                ...
                                                <FormControl>
                                                    ...
                                                    >
                                                        ...
                                                        <FormItem><FormControl><RadioGroupItem value="lanyard" className="sr-only" /></FormControl><FormLabel className={cn("flex flex-col items-center justify-center rounded-md border-2 p-4 cursor-pointer hover:bg-accent hover:border-primary", field.value === 'lanyard' && 'border-primary')}><div className="w-12 h-[75px] bg-muted rounded-md flex flex-col items-center justify-center p-1 gap-1 relative"><div className="absolute top-2 left-1/2 -translate-x-1/2 w-4 h-4 rounded-full border-2 border-primary/50 bg-background" /><div className="w-6 h-6 rounded-full bg-primary/20 mt-4" /><div className="w-full space-y-1 mt-1"><div className="h-1.5 w-3/4 mx-auto bg-primary/50 rounded-full" /><div className="h-1.5 w-2/3 mx-auto bg-primary/50 rounded-full" /></div></div><span className="mt-2 font-semibold">Lanyard</span></FormLabel></FormItem>
                                                    </RadioGroup>
                                                </FormControl>
                                                <FormMessage />
                                                </FormItem>
